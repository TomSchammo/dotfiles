...
# TODO either install minimal init.vim or standard init.vim
...
do
    case $REPLY in
        ...
        2)
            ...
            break;;
        3)
            echo "Downloading Binary is not supported yet"
            exit 1
            break;;
        4)
            echo "Installing neovim using the snap store is not supported yet"
            exit 1
            break;;
        5)
            echo "Quitting..."
...
# wget to download my init.vim from github
sudo ${PKG_MNGR_INSTALL} wget
...
sudo ${PKG_MNGR_INSTALL} python python-pip
...
sudo ${PKG_MNGR_INSTALL} texlive-most
...
# if there already is an init.vim it is saved
...
(cat <<EOF
{
  ...
  "clangd.arguments": ["--background-index", "-j=4", "--cross-file-rename", "--suggest-missing-includes"],
  "cSpellExt.enableDictionaries": ["german"],
  "cSpell.language": "de,en",
    "cSpell.userWords": [
  ],
  "snippets.userSnippetsDirectory": "/home/tom/.config/nvim/custom_snippets",
  "languageserver": {
    "cmake": {
